...
| RT-016 | "Our Spark job runs forever — one task never finishes while the rest are done." | Big-data / skew | `COMPASS.md` → `docs/knowledge-map.md` (symptom: Big-data) → `skills/bi-bigdata-knowledge/SKILL.md` → `skills/bi-bigdata-knowledge/INDEX.md` → "Symptom routes" | `join-skew-checklist` verdict (skew / hot key diagnosis) | Do not run/tune a real job; do not `collect()` to "check"; do not invent the KPI's meaning here. | Not run |
| RT-017 | "Which engine should we use to aggregate 2 TB of sales to daily-branch grain?" | Big-data / engine selection | `COMPASS.md` → `docs/knowledge-map.md` (route 17a) → `skills/bi-bigdata-knowledge/SKILL.md` → `skills/bi-bigdata-knowledge/INDEX.md` → `knowledge/engine-selection.md` | `engine-selection-checklist` (may legitimately recommend pushing down to SQL/warehouse) | Do not default to Spark; do not scale out without confirming single-node won't do; do not redefine grain/additivity (that is the KPI contract). | Not run |
| RT-018 | "Clean and standardize this 50 MB CSV of categories." | Single-node (boundary case — routes AWAY from Big-data) | `COMPASS.md` → `docs/knowledge-map.md` (route 14) → `skills/bi-python-knowledge/SKILL.md` → `skills/bi-python-knowledge/INDEX.md` | cleaning review artifact | Do not route to Big-data — it fits on one machine; Big-data must bounce single-node work back to Python. | Not run |
...
- defines a KPI's business meaning inside the DAX (or SQL/Python/Big-data) layer instead of
  routing to `skills/retail-kpi-knowledge/` first;
- routes single-node-sized work to Big-data, or scales out without confirming single-node
  won't do (Big-data must bounce "this fits on one machine" back to Python);
- runs / tunes a real distributed job, or `collect()`s a distributed result to "check" it
  (Big-data is reasoning only);
