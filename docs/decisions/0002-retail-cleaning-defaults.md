...
- **D1 — Model at the lowest grain the source provides.** Default to the line-item /
...
- **D2 — Verify the primary key on the data, and re-verify on the transformed output.**
  Raw uniqueness can break after TRIM/cast. *(C086: PK held on all 246,916 transformed rows.)*

### Columns & PII

- **D3 — Drop columns that carry no signal:** 100%-empty, single-value, verified
...
- **D4 — Remove personal/sensitive data before the BI layer — decided EARLY, not at
...
- **D5 — Treat the empty string as missing.** Convert `''` → NULL on all columns first;
  measure missingness as `'' OR NULL`, never `IS NULL` alone.
- **D6 — Fill policy:** NULL for genuinely-unknown facts; a sentinel
...
- **D7 — Money/quantities → exact `NUMERIC`, never float. Dates → `DATE` if no real time
...
- **D8 — Keep returns, and derive an `is_return` boolean from the authoritative column
...
- **D9 — Keep the independent money measures (gross, net, tax, discount); drop only true
...
- **D10 — Unify categorical encodings to one standard** (one language, one case). Keep
  the original code as a separate column when it is a stable join key.
- **D11 — Add business rollups only from an analyst-supplied mapping; never invent the
...
- **D12 — Model a non-tree hierarchy as flat denormalized levels, not a snowflake.** When
...
- **D13 — Materialize silver as a TABLE (not a view) via an idempotent numbered
...
- **D14 — Gold is a Kimball star:** one fact at the silver grain + conformed dims;
...
- **D15 — The date dimension is a CONTIGUOUS generated calendar over the full span,**
  never `SELECT DISTINCT date` (missing days break time-intelligence).
- **D16 — Reconcile measure totals at every layer** (source → silver → gold → BI) and
...
Any new retail table starts from D1–D16 as defaults and only documents **deviations**,
