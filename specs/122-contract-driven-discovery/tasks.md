...
- [ ] T016 [US2] Add the `domain_guess`-stage local stop **in the skill's own logic** (NOT via the inherited gate -- the gate returns `pass` on an absent/empty store for this empty-`blocking_decision_categories` stage, FR-025): if no committed survey exists, the skill checks its own precondition and halts with a truthful local stop naming the missing survey. Green the T014 domain tests; `retail check` stays exit 0
