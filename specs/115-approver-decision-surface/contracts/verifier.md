...
- **V1 completeness (SAFETY)**: every refusal-eligible source item is present in
  ...
  dropped.
  - **Independent oracle (do NOT make this circular).** The stage-side ground
    truth is enumerated from `status_yaml` via plain `yaml.safe_load` --
    independent of the composer, so V1 is a real oracle there. The question-side
    ground truth MUST likewise be INDEPENDENT of the production markdown-table
    parser: `questions_rows` passed to the verifier is a HAND-AUTHORED expected
    open-row set (literal ids/count declared in the fixture-test), NEVER the
    output of `approver_view`'s own parser. Otherwise a parser bug that silently
    drops an open row drops it from BOTH the composer output and the expectation,
    and completeness passes vacuously while the signer never sees that open
    governance question. (This is spec 114's join-defect failure-class reproduced
    on the questions side; the guarantee must sit ON the untrusted parse, not
    beside it.)
  - **Parser-under-test fixture (required).** Include at least one fixture whose
    `unresolved-questions.md` has an AWKWARD open row -- an embedded `|` inside a
    cell, extra whitespace/alignment padding, and a near-miss status string
    (e.g. `open ` with trailing space, or `Answered?`) -- with the expected open
    set hand-declared. This proves the markdown-table parser (U1) is what the
    completeness check exercises, so a parse regression FAILS a test rather than
    passing silently.
...
| awkward-questions (parser-under-test) | an OPEN row with an embedded `|`, alignment padding, and a near-miss status string; expected open set hand-declared | the open row is present in refusal_case; a parse regression FAILS V1 (independent oracle) |
