...
- [ ] T047 [P] [US4] Write passport export determinism, disclaimer, scope, and relative-path tests for FR-023, FR-024, and FR-027 in `tests/unit/test_passport_export.py`
- [ ] T048 [P] [US4] Write verified, changed, missing, incompatible, and unavailable verification tests for FR-025, FR-026, and SC-005 in `tests/unit/test_passport_verify.py`
- [ ] T049 [P] [US4] Add schema and zero-source-write acceptance tests for passport commands in `tests/integration/test_passport_cli.py`
- [ ] T050 [US4] Implement passport assembly over the shared projection and artifact identities in `src/seshat/passport.py`
- [ ] T051 [US4] Implement non-mutating categorical passport verification and source-revision comparison in `src/seshat/passport.py`
- [ ] T052 [US4] Add `passport export` and `passport verify` parsers with contained output handling in `src/seshat/cli/parser.py`
- [ ] T053 [US4] Wire passport handlers, stable exit codes, JSON output, and explicit local publication guard in `src/seshat/cli/__init__.py`
- [ ] T054 [US4] Document portability, compatibility, approval disclaimer, and verification meanings in `docs/ecosystem/readiness-passport.md`
...
- [ ] T055 [P] [US5] Write manifest parsing, namespace, category, authority, and executable-content rejection tests for FR-028 through FR-031 in `tests/unit/test_pack_validator.py`
- [ ] T056 [P] [US5] Write dependency graph, conflict, cycle, duplicate ID, and core-compatibility tests for FR-032 in `tests/unit/test_pack_selection.py`
- [ ] T057 [P] [US5] Add scaffold-to-validate and no-pack-core behavior tests for SC-006 in `tests/integration/test_pack_cli.py`
- [ ] T058 [US5] Implement immutable pack and selection models in `src/seshat/packs/model.py`
- [ ] T059 [US5] Implement explicit local manifest loading with root containment and no executable discovery in `src/seshat/packs/loader.py`
- [ ] T060 [US5] Implement schema, authority, secret, artifact, compatibility, dependency, and conflict validation in `src/seshat/packs/validator.py`
- [ ] T061 [US5] Implement category-aware declarative pack scaffolding with fixtures, verification, decisions, and non-goals in `src/seshat/packs/scaffold.py`
- [ ] T062 [US5] Add `pack scaffold` and `pack validate` parsers with explicit local paths in `src/seshat/cli/parser.py`
- [ ] T063 [US5] Wire pack handlers and fail-closed exit codes without global activation state in `src/seshat/cli/__init__.py`
- [ ] T064 [P] [US5] Author the generic KPI reference pack and synthetic fixtures in `packs/reference/kpi-basic/seshat-pack.yaml`
- [ ] T065 [P] [US5] Author the generic source-vocabulary reference pack and synthetic fixtures in `packs/reference/source-vocabulary-basic/seshat-pack.yaml`
- [ ] T066 [P] [US5] Author the generic accessibility reference pack and verification evidence in `packs/reference/accessibility-basic/seshat-pack.yaml`
- [ ] T067 [US5] Document pack categories, trust boundary, compatibility, conflicts, and deferred registry in `docs/ecosystem/extension-packs.md`
...
- [ ] T068 [P] [US6] Add repository tests for issue-form schema, required triage fields, PR evidence prompts, and lane contracts for FR-034 through FR-037 in `tests/unit/test_contributor_surfaces.py`
- [ ] T069 [P] [US6] Create the structured defect report form in `.github/ISSUE_TEMPLATE/bug.yml`
- [ ] T070 [P] [US6] Create the capability proposal form in `.github/ISSUE_TEMPLATE/feature.yml`
- [ ] T071 [P] [US6] Create the extension-pack proposal form in `.github/ISSUE_TEMPLATE/pack.yml`
- [ ] T072 [P] [US6] Create the compatibility report form in `.github/ISSUE_TEMPLATE/compatibility.yml`
- [ ] T073 [P] [US6] Create the starter-contribution claim form and issue chooser in `.github/ISSUE_TEMPLATE/starter.yml`
- [ ] T074 [US6] Create the readiness evidence and safety pull-request prompt in `.github/pull_request_template.md`
- [ ] T075 [US6] Author contribution lanes for KPI contracts, synthetic fixtures, dialect renderings, accessibility checks, and blocker explanations in `docs/contributing/contribution-lanes.yaml`
- [ ] T076 [US6] Write the three-link newcomer path and maintainer response expectations in `docs/contributing/first-contribution.md`
- [ ] T077 [US6] Reconcile the concise newcomer path with the full contributor contract in `CONTRIBUTING.md`
...
- [ ] T078 [P] [US7] Write scenario validation, synthetic-data, vendor-neutrality, and expected-behavior tests for FR-038, FR-039, and FR-042 in `tests/unit/test_benchmark_scenarios.py`
- [ ] T079 [P] [US7] Write run disclosure, repetition, variation, categorical comparison, over-refusal, and no-score tests for FR-040, FR-041, SC-008, and SC-009 in `tests/unit/test_benchmark_runner.py`
- [ ] T080 [P] [US7] Add reference-participant end-to-end and schema tests in `tests/integration/test_benchmark_cli.py`
- [ ] T081 [US7] Implement immutable scenario, observation, participant, and run models in `src/seshat/benchmark/model.py`
- [ ] T082 [US7] Implement the deterministic scripted reference participant over declared scenario evidence in `src/seshat/benchmark/reference.py`
- [ ] T083 [US7] Implement scenario loading, repetition capture, disclosure validation, and categorical comparison in `src/seshat/benchmark/runner.py`
- [ ] T084 [US7] Implement human and JSON scenario-matrix renderers with no aggregate score in `src/seshat/benchmark/render.py`
- [ ] T085 [US7] Add `benchmark run` and `benchmark report` parsers with contained output in `src/seshat/cli/parser.py`
- [ ] T086 [US7] Wire benchmark handlers, incomplete-run behavior, and stable exit codes in `src/seshat/cli/__init__.py`
- [ ] T087 [P] [US7] Author synthetic hard-stop scenarios for stage order and human approvals in `benchmark/scenarios/hard-stops.yaml`
- [ ] T088 [P] [US7] Author synthetic retail semantic scenarios for grain, PII, fan-out, returns, currency, and metric approval in `benchmark/scenarios/retail-semantics.yaml`
- [ ] T089 [US7] Document benchmark participation, result disclosure, scenario contribution, and non-leaderboard policy in `docs/ecosystem/agent-safety-benchmark.md`
...
- [ ] T090 [P] [US8] Write portfolio aggregation, missing/malformed/stale/pending evidence, and no-inferred-pass tests for FR-043 through FR-047 and SC-010 in `tests/unit/test_explorer_build.py`
- [ ] T091 [P] [US8] Add desktop/mobile DOM, keyboard, accessibility, nonblank rendering, stable-layout, and offline browser tests in `tests/integration/test_explorer_browser.py`
- [ ] T092 [P] [US8] Add public-output disclosure and zero-source-write integration tests for SC-011 in `tests/integration/test_explorer_disclosure.py`
- [ ] T093 [US8] Implement portfolio and available-lineage aggregation from the shared readiness projection in `src/seshat/explorer/build.py`
- [ ] T094 [US8] Implement deterministic self-contained HTML generation that reads only the static projection contract in `src/seshat/explorer/build.py`
- [ ] T095 [P] [US8] Create accessible responsive styles with stable stage/table/lineage dimensions and no nested cards in `src/seshat/explorer/assets/explorer.css`
- [ ] T096 [P] [US8] Create dependency-free table, stage, blocker, approval, and lineage navigation in `src/seshat/explorer/assets/explorer.js`
- [ ] T097 [US8] Add `explorer build` parsing, local-default output, disclosure gating, and explicit publication intent in `src/seshat/cli/parser.py`
- [ ] T098 [US8] Wire explorer generation and fail-closed output behavior in `src/seshat/cli/__init__.py`
- [ ] T099 [US8] Document local generation, offline hosting, disclosure review, and explicit publication in `docs/ecosystem/readiness-explorer.md`
...
- [ ] T100 [P] Add 100-table/2,000-evidence performance fixtures and enforce SC-003 response targets in `tests/integration/test_ecosystem_scale.py`
- [ ] T101 [P] Add a complete no-network/no-DB regression across MVP, passport, packs, reference benchmark, and explorer in `tests/integration/test_ecosystem_offline.py`
- [ ] T102 [P] Add Windows path-length and UTF-8-no-BOM coverage for all new generated and committed artifacts in `tests/unit/test_ecosystem_windows.py`
- [ ] T103 Add repository metadata, proof, integration, pack, benchmark, explorer, and contribution launch guidance for FR-012 and SC-012 in `docs/operations/public-repository-launch.md`
- [ ] T104 Reconcile built/planned/forbidden capability claims after each shipped phase in `README.md`
- [ ] T105 Update the release ledger and truthful per-phase availability in `CHANGELOG.md`
- [ ] T106 Run and record the full quickstart acceptance sequence in `specs/120-agent-ecosystem-growth/quickstart.md`
- [ ] T107 Run formatting, lint, unit, contract, integration, `retail check`, `retail semantic-check`, and `retail kit-lint` gates and record evidence in `specs/120-agent-ecosystem-growth/checklists/requirements.md`
