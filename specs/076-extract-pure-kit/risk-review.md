...
| MAJOR-1 | "No real host committed" was FALSE — cluster id `db-<cluster-id>` + `ezaby_demo` in 7 files, evading the C2 FQDN-only regex AND the grep | **FIXED** — finding corrected in spec/plan/research; cluster id + `ezaby_demo` added to redaction targets + acceptance markers; C2 FQDN-only gap noted. |
