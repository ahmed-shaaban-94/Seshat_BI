...
- **FR-005**: The feature MUST be surfaced as a read-only skill/composer over a reusable library function, honoring the ratified Option B decision; it MUST NOT add a new top-level CLI verb.
...
- The bundle is surfaced as a read-only skill over a reusable library function, applying the ratified Option B policy (2026-07-07); no new top-level CLI verb is added. The peer `explorer`/`passport` verbs (added by spec 120, created 2026-07-11, i.e. after the ratification) are surfaced for the ratifier as a verb-parity counter-argument, not treated as a grandfathered exception. Reversible-but-costly if the owner later directs a verb.
