...
1. **Delivery shape (verb vs skill)** -- RESOLVED: skill over a library function (Option B ratified 2026-07-07). The explorer/passport CLI verbs predate that ratification and are grandfathered; this feature does not add a new verb.
2. **Redact vs fail-closed tension (req 4 vs req 7)** -- RESOLVED: live secret/DSN/PII/abs-path findings block generation (reuse `scan_disclosure`, inherit Explorer posture); "redacted" in the manifest names by-design portability normalizations only (abs-path -> repo-relative, private URL stripped).
