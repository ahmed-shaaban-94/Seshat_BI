...
| C2 | Constitution latent-gap | MEDIUM | data-model dimension->source map + readiness_classify rank | The shipped rank buckets are `approval > grain > live_validation > artifact > readiness` -- there is NO `pii` bucket. If the rank were ever applied to relayed drift CLASSES (per the broad D4 reading), a `pii_surface_drift` blocker (the most dangerous Principle-V class) would fall to the catch-all and rank LOWEST. | Bounded, not a blocker: FR-006 already flags `requires_human_attention` for ANY relayed Principle-V drift regardless of rank, so PII is never silently deprioritized in the summary. Record as an implement-time verify: keep PII-class relays on the attention flag, not dependent on the rank bucket. Do NOT invent a new rank bucket (that would edit a shipped committed lookup, hard rule #9). |
