...
``_current_engine``, ``_safe_target_label``) via ``from retail import cli`` and
``cli.<name>(...)`` -- NOT a by-value import -- so ``monkeypatch.setattr(
"retail.cli._make_runner", ...)`` in the test suite still lands on the exact
...
def run_validate(args: argparse.Namespace) -> int:
    ...
    from retail.runner import _format  # reuse the [severity] id (locator) format
    from retail.validate import resolve_dsn, run_live_checks
